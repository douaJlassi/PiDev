package controllers;
import services.WeatherServiceSahar;
...
public class DashboardGuideController {
    ...
    private WeatherServiceSahar weatherServiceSahar = new WeatherServiceSahar();
...
// ═════════════════════════════════════════════════════════════════════════════
    ...
    private Map<String, Integer> fetchMonthlyCount(String sql, int year) {
        ...
        try (Connection c = MyDBConnexion.getInstance().getConnection();
...
// ═════════════════════════════════════════════════════════════════════════════
    ...
    private List<ReservationDetail> getReservationsForActivity(int idActivite) {
        ...
        String sql = "SELECT c.nom, c.prenom, c.email, c.telephone, a.nbPlaces, a.montantTotal " +
                "FROM achat a " +
                "JOIN clients1 c ON a.idClient = c.idUser " +
    ...
    private void loadWeatherForActivity(Activite a, Label label, ImageView icon) {
        ...
        weatherServiceSahar.getWeatherForCityAndDateTime(a.getLieu(), activityDateTime)
