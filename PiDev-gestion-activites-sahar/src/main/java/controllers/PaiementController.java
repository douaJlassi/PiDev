...
public class PaiementController {
    ...
    private Consumer<Boolean> onResult; // true = paid, false = cancelled

    // ─────────────────────────────────────────────
    //  Public API — call before showing the stage
    // ─────────────────────────────────────────────
    ...
    private void handlePay() {
        ...
        // ── Option A: Simulation (no real API) ──
        // new Thread(() -> simulatePayment()).start();

        // ── Option B: Real Stripe API ──
        new Thread(() -> processWithStripe()).start();
    }
    ...
    private void processWithStripe() {
        try {
            com.stripe.Stripe.apiKey = StripeConfig.SECRET_KEY; // see StripeConfig.java
            ...
            piParams.put("return_url", "https://your-app.com/return"); // needed for 3DS
