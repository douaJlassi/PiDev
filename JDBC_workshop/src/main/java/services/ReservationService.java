...
public class ReservationService implements CRUDservices<String, reservation> {
    ...
    public void deletebyId(int id) throws SQLException {
        String req = "DELETE FROM `reservations` WHERE `id_reservation` = " + "'" + id + "'";
        PreparedStatement ps = connection.prepareStatement(req);
        ps.executeUpdate();
    }
    ...
    public List<Integer> selectSeats(int ServiceId) throws SQLException {
        ...
        String req = "SELECT seat_nb FROM reservations WHERE `idService` = '" + ServiceId + "'";
        try (Statement st = connection.createStatement();
             ResultSet rs = st.executeQuery(req)) {
    ...
    private void verifierDoublonHotel(String nomClient, Date dateReservation) throws SQLException, ReservationException {
        String req =
                "SELECT r.id_reservation, s.nom AS nomHotel " +
                        "FROM reservations r " +
    ...
    private void verifierDoublonVol(String nomClient, int idVolNouveau) throws SQLException, ReservationException {
        ...
        String reqVol = "SELECT date_depart, date_arrive FROM services WHERE idService = ?";
        ...
        try (PreparedStatement pst = connection.prepareStatement(reqVol)) {
            ...
            departNouv = rs.getDate("date_depart");
            arriveeNouv = rs.getDate("date_arrive");
        }

        // Chercher un vol existant du même client dont les horaires se chevauchent
        String req =
                "SELECT s.nom AS nomVol, s.date_depart, s.date_arrive " +
                        "FROM reservations r " +
                        "JOIN services s ON r.idService = s.idService " +
                        "WHERE r.nom = ? " +
                        "  AND s.type = 'vol' " +
                        "  AND s.idService <> ? " +
                        "  AND NOT (s.date_arrive < ? OR s.date_depart > ?)";
            ...
            if (rs.next()) {
                throw new ReservationException(
                        "❌ Conflit horaire : vous avez déjà le vol « " + rs.getString("nomVol") +
                                " » (du " + rs.getDate("date_depart") + " au " + rs.getDate("date_arrive") +
    ...
    private String getVilleArriveeVol(int idVol) throws SQLException {
        String req = "SELECT ville_arrivee FROM services WHERE idService = ?";
        try (PreparedStatement pst = connection.prepareStatement(req)) {
            pst.setInt(1, idVol);
            ResultSet rs = pst.executeQuery();
            return rs.next() ? rs.getString("ville_arrivee") : null;
        }
    }
    ...
    private void verifierCoherenceVolHotel(String nomClient, int idServiceNouveau, String typeNouv) throws SQLException, ReservationException {
        ...
        if (typeNouv.equals("vol")) {
            ...
            String req =
                    "SELECT s.nom AS nomVol, s.ville_arrivee " +
                            "FROM reservations r " +
                            "JOIN services s ON r.idService = s.idService " +
    ...
    public void validerReservation(int id) throws SQLException {
        String req = "UPDATE reservations SET  statut = ? WHERE id_reservation ='" + id + "'";
        try (PreparedStatement pst = connection.prepareStatement(req)) {
    ...
    public int getIdReservation(reservation reservation) throws SQLException {
        ...
        String req = "SELECT id_reservation FROM reservations "
