...
public class ChatView {
    ...
    @FXML
    private ListView<Person> listAllUsers;
    @FXML
    ...
    private ObservableList<Person> masterUserList = FXCollections.observableArrayList();
    private FilteredList<Person> filteredUserList;
    ...
    private Set<Person> selectedUsers = new HashSet<>();
    @FXML
    ...
    public void initialize() {
        this.userConnecte = SessionManager.getCurrentUser();

        if (userConnecte != null) {
            this.currentUserId = userConnecte.getId();
            System.out.println("✅ Session active : " + userConnecte.getUsername());
        } else {
    ...
    private void afficherDiscussion(Conversation conv) {
        ...
        try {
            ...
            List<Messages> messages = serMsg.selectByConversation(conv.getIdConversation(),currentUserId);
            for (Messages m : messages) {
                boolean isMoi = (m.getExpediteur().getId() == currentUserId);
    ...
    private void prepareUserSelectionList() {
        ...
        try {
            List<Person> allUsers = serUser.selectALL();
            ...
            listAllUsers.setCellFactory(lv -> new ListCell<Person>() {
                @Override
                protected void updateItem(Person user, boolean empty) {
                    ...
                    if (empty || user == null) {
                        ...
                        String initialesText = (user.getLastName().substring(0, 1) + user.getName().substring(0, 1)).toUpperCase();
                        ...
                        Label lblName = new Label(user.getLastName() + " " + user.getName());
            ...
            listAllUsers.setOnMouseClicked(event -> {
                Person selection = listAllUsers.getSelectionModel().getSelectedItem();
    ...
    public void handleConfirmGroupSelection() throws SQLException {
        ...
        dialog.showAndWait().ifPresent(groupTitle -> {
            ...
            try {
                ...
                for (Person member : selectedUsers) {
    ...
    private void creerOuOuvrirDiscussion(Person destinataire) {
        try {
            Conversation existing = serConv.findPrivateChat(currentUserId, destinataire.getId());
    ...
    private void renderMessage(Messages msg, boolean isMoi) {
        ...
        String senderName = msg.getExpediteur().getLastName() + " " + msg.getExpediteur().getName();
        ...
        Label lblSenderName = new Label(msg.getExpediteur().getLastName() + " " + msg.getExpediteur().getName());
    ...
    private void chargerHistorique(int idConversation) {
        vboxMessages.getChildren().clear();
        try {
            List<Messages> historique = serMsg.selectByConversation(idConversation, currentUserId);

            for (Messages m : historique) {
                boolean isCurrentUser = (m.getExpediteur().getId() == currentUserId);
                ...
                if (empty || conv == null) {
                    ...
                    try {
                        ...
                        if (last != null) {
                            ...
                            if (!last.isLu() && last.getExpediteur().getId() != currentUserId) {
    ...
    private void handleVoirMembres(Conversation conv) {
        ...
        try {
            ServiceParticipantConversation spc = new ServiceParticipantConversation();
            List<Person> membres = spc.getParticipantsByConversation(conv.getIdConversation());

            for (Person u : membres) {
                HBox row = new HBox(10);
                row.setAlignment(Pos.CENTER_LEFT);
                Label name = new Label(u.getLastName() + " " + u.getName());
    ...
    private void handleAjouterMembre(Conversation conv) {
        ...
        userListView.setOnMouseClicked(event -> {
            Person selectedUser = userListView.getSelectionModel().getSelectedItem();
            if (selectedUser != null) {
                Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
                confirm.setTitle("Confirmation");
                confirm.setHeaderText("Ajouter " + selectedUser.getLastName() + " au groupe ?");
                ...
                if (confirm.showAndWait().get() == ButtonType.OK) {
                    try {
                        ...
                        new Alert(Alert.AlertType.INFORMATION, selectedUser.getLastName() + " a rejoint le groupe !").show();
                    } catch (SQLException e) {
        ...
        for (String e : emojis) {
            ...
            lblEmoji.setOnMouseClicked(ev -> {
                try {
                    ...
                    boolean isMoi = (msg.getExpediteur().getId() == currentUserId);
    ...
    private void handleSendLocation() {
        ...
        btnValider.setOnAction(e -> {
            ...
            if (lat != null && !lat.toString().equals("null")) {
                // 1. On prend une "photo" de la carte
                WritableImage snapshot = webView.snapshot(new SnapshotParameters(), null);
                String imgName = "map_" + System.currentTimeMillis() + ".png";

                try {
                    // --- CORRECTIF : CRÉER LE DOSSIER S'IL N'EXISTE PAS ---
                    File directory = new File(UPLOAD_DIR);
                    if (!directory.exists()) {
                        directory.mkdirs(); // Crée le dossier 'uploads' à la racine
                    }
                    // ------------------------------------------------------

                    File file = new File(directory, imgName);

                    // 2. Écriture du fichier sur le disque
                    // On utilise SwingFXUtils pour convertir l'image JavaFX
                    boolean success = javax.imageio.ImageIO.write(
                    ...
                    if (success) {
                        // 3. Envoyer les données au format coords|image
                        envoyerMessageLocation(lat.toString() + "," + lon.toString() + "|" + imgName);
                        mapStage.close();
                    } else {
                        System.err.println("Échec de l'enregistrement de l'image.");
                    }

                } catch (Exception ex) {
                    System.err.println("Erreur fatale lors de l'écriture : " + ex.getMessage());
                    ex.printStackTrace();
                }
            } else {
                new Alert(Alert.AlertType.WARNING, "Position non détectée. Cliquez sur la carte.").show();
            }
        });
        ...
        // Rafraîchissement de la carte pour éviter les carrés gris
        engine.getLoadWorker().stateProperty().addListener((obs, oldState, newState) -> {
            if (newState == javafx.concurrent.Worker.State.SUCCEEDED) {
                Platform.runLater(() -> engine.executeScript("fixMap()"));
            }
        });
    }
