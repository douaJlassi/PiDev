...
public class VoyageurOffersGridController implements OfferFilterAware {
    ...
    private void setupAutoHideActualitesOnScroll() {
        if (offersScroll == null || actualitesContainer == null) return;

        // ✅ hysteresis thresholds (prevents jumpy flicker)
        final double SHOW_AT = 0.01;  // show only when very top
        final double HIDE_AT = 0.05;  // hide after scrolling a bit

        offersScroll.vvalueProperty().addListener((obs, oldV, newV) -> {
            double v = newV.doubleValue();

            if (!actualitesVisible && v <= SHOW_AT) {
                showActualitesSmooth();
            } else if (actualitesVisible && v >= HIDE_AT) {
    ...
    private void hideActualitesSmooth() {
        ...
        FadeTransition fade = new FadeTransition(Duration.millis(100), actualitesContainer);
        fade.setFromValue(1.0);
        fade.setToValue(0.0);

        TranslateTransition slide = new TranslateTransition(Duration.millis(100), actualitesContainer);
        slide.setFromY(0);
        slide.setToY(-12);
    ...
    private void showActualitesSmooth() {
        ...
        // must be managed+visible before animating
        ...
        FadeTransition fade = new FadeTransition(Duration.millis(180), actualitesContainer);
        fade.setFromValue(0.0);
        fade.setToValue(1.0);

        TranslateTransition slide = new TranslateTransition(Duration.millis(180), actualitesContainer);
        slide.setFromY(-12);
        slide.setToY(0);
    ...
    private void loadActualites() {
        if (actualitiesBox == null) return;

        actualitiesBox.getChildren().clear();

        List<Actualite> list = actualiteRepo.findAllActive();

        for (Actualite a : list) {
            actualitiesBox.getChildren().add(createBannerCard(a));
        }
    }

    private StackPane createBannerCard(Actualite a) {
    ...
    """);
        ...
        // ✅ Click banner => open offer details
        card.setOnMouseClicked(e -> {
            actualiteRepo.incrementClick(a.getIdActualite());
            openOfferDetails(a.getIdOffre());
        });

        return card;
    }

    private void openOfferDetails(int idOffre) {
        try {
